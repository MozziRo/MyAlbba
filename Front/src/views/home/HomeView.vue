...
<div v-if="index === 0 || index === 3" class="content-center fadeInUp">
  <h1 class="slide-title fadeInUp" v-if="index === 0">
    <span class="highlight">{{ slide.title[0] }}</span>{{ slide.title[1] }}
  </h1>
  <h1 class="slide-title fadeInUp" v-else>{{ typeof slide.title === 'string' ? slide.title : slide.title.join('') }}</h1>
  <p class="slide-description fadeInUp">{{ slide.description }}</p>
      ...
      <div v-if="index === 1" class="content content-left fadeInUp">
        ...
        <div class="text-container">
          <h1 style="width: max-content" class="slide-title fadeInUp">
            {{ slide.title }}
          </h1>
      ...
      <div
        v-if="index === 4"
        style="width: 1000px"
        class="content content-right fadeInUp video"
      >
        <div class="text-container">
...
const slides = ref([
   { title: ['내알빠다', '에 오신 것을 환영합니다!'], description: "근무 관리의 새로운 기준" },
...
/* Chrome, Safari, Opera 용 스크롤바 숨기기 */
